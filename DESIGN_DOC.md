...
> 项目：Voice Canvas：基于三层智能路由的 AI 语音绘图工具  
...
```mermaid
flowchart TD
    A["语音识别文本"] --> B["第一层：本地快速命令"]
    B -->|命中| C["前端直接生成 Draw Commands"]
    B -->|未命中| D["第二层：场景模板"]
    D -->|命中| E["SceneExecutor 生成场景对象"]
    D -->|未命中| F["第三层：LLM 工具规划"]
    F --> G["DrawingExecutor 生成 Draw Commands"]
    C --> H["Canvas Store 执行"]
    E --> H
    G --> H
    H --> I["Konva 画布渲染"]
...
计划用于处理稳定的常见场景，让比赛演示中的复杂场景生成更可靠。
...
- `SceneExecutor` 将场景计划转换为前端可执行命令。
- 场景对象保留 `kind`、`label`、`role` 等语义信息，便于后续继续编辑。

### 第三层：LLM 工具规划

计划用于处理开放式对象创建、复杂编辑和模板无法覆盖的自然语言请求。
...
- 使用 Pydantic 校验工具参数。
- 使用 `DrawingExecutor` 将工具计划转换为画布命令。
- 支持 SVG 素材解析与匹配。
- 支持目标查询和歧义候选返回。
